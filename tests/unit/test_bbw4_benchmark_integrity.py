...
# descriptive run EXACTLY today (delta = 0.0 for all four components); 1e-4 %/mo
# (= 0.01 bp/mo) is immune to any float-repr noise yet fails on any material
# builder drift.
...
pytestmark = pytest.mark.skipif(
    ...
    reason="BBW-4 factor parquets or the committed descriptive run are absent "
    "(data/ not built in this checkout)",
)


def _expected_level_pct(factor: str) -> float:
    """The frozen corr / total_return mean (%/mo) from Workstream-D's committed run."""
...
def test_bbw4_component_level_matches_committed_run(factor):
